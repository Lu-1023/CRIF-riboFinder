...
library(readr)
library(ggrepel)
setwd("/media/hp/disk1/lu/demo/b_align/noscreen/FS")
samples<-system("ls /media/hp/disk1/lu/demo/b_align/noscreen/coverage_stat/*_RPF_count.txt",intern=T)
#RPF aim to add the RPKM and shift_ratio
RPF <- function(sam,f)  {
  #sam:the path of the RPF_count.txt, f:frame shifting sorts<1 or 2>
  x<-gsub(".*/(.*)_RPF_count.txt","\\1",sam)
