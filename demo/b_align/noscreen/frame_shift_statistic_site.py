...
def R_box(x,y,z): #x : motif stop site .y: +1 or -1 hiden stop codon site .z: frame.
    r_box=[]
    global num
    num=0
...
def find_seq(x):
    hsc=open(organism +"_ALL_hsc_position_frame"+str(x)+".txt","r")
    h={}
    for l in hsc:
        if (re.match(r'^id',l)):
            continue
...
def pro_bg (x,y):
    ...
    bg_file=open(y+"/b_align/noscreen/"+x+"_A.bg",'r')
    for L in bg_file:
        a=L.strip('\n').split('\t')
    ...
    hsc = open(organism + "_ALL_hsc_position_frame"+str(frame)+".txt", 'r')
    for l in hsc:
        l = l.strip('\n')
        if (re.match(r'^id', l)):
            continue
        ...
        if (id in gene_exist):
            ...
            for i in range(100+30+frame,size[id]-100-30,3):
                if (id+"_"+str(i) in rpf.keys()) :
                    ...
                    baseline.append(0)
            #value avg(all -1)
            length = (size[id] - 200-60) / 3  # codon length
