...
setwd("/media/hp/disk1/lu/tissue_test/b_align/noscreen/FS_plot")
...
plot.bin.fs<-function(x,y,s){
  ...
  if (s==1){
    fs<-read.table(paste0(x,z,"frame",y,"_1_codon3.txt"),header = F,sep="\t")
  }else if (s==2){
    fs<-read.table(paste0(x,z,"frame",y,"_2_codon3.txt"),header = F,sep="\t")
  }else{
    fs<-read.table(paste0(x,z,"frame",y,"_nc_codon3.txt"),header = F,sep="\t")
  }
  # 1 median normalized
