...
mkdir 'codon_stat_noscreen';
mkdir 'cleavage_preference_noscreen';
mkdir 'noscreen';

open (REF, "/media/hp/disk1/song/Genomes/$species/$species\_ref/longest_cDNA.fa") or die $!;
my %si; my %gene; my $ge; 
while (<REF>) {
    $_=~ s/\s+$//;
    if (/^>/)       {
...
sub trimbed                       {
  ...
  while (<hand0>)                  {
    ...
    my $m=max(@a[2..4]);
    if ($a[2] == $m)  {$trim{$a[0]} = 15;}
    elsif ($a[3] == $m)  {$trim{$a[0]} = 14;}
    elsif ($a[4] == $m)  {$trim{$a[0]} = 16;}
                                    }
  close hand0;
  ...
  open (hand3, ">noscreen/$sam\_A.bed");
  open (hand4, ">codon_stat_noscreen/$sam\_trim.txt");
  open (hand5, ">cleavage_preference_noscreen/$sam.5end.stat.txt");
